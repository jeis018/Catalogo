...
if (isset($_GET['id']) && isset($_GET['ac'])) {
    ...
    $directorio = '../reports/PlantillaOrden.xls';
    ...
    header('Content-Disposition: attachment;filename="' . $fileReport . '.xls"');
    ...
    if ($ac === 'D') {
        ...
        $objWriter->save('../emailOrders/' . $fileReport . '.xls');
        $mailReport = new ReportMail();
//        $mailReport->sendReportMail($fileReport . '.xls', $resultInsert);
        $mailReport->sendReportMail($fileReport . '.xls', __FILE__);
    }
    exit;
}